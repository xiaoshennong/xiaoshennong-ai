...
# 生成更多医案（目标：50,000个）
def generate_more_cases(count=50000):
    """生成大量医案数据"""
    cases = []
    ...
    for i in range(count):
        ...
        case = {
            'case_id': f'PT-{datetime.now().strftime("%Y%m%d")}-{i+1:06d}',
...
# 生成更多古籍条文（目标：50,000条）
def generate_more_texts(count=50000):
    """生成大量古籍条文"""
    texts = []
...
def main():
    print("=" * 60)
    print("小神农中医AI - 批量数据生成器")
    print("目标：生成100,000+条数据")
    print("=" * 60)
    ...
    # 生成医案（50,000个）
    print("\n[1] 生成医案...")
    cases = generate_more_cases(50000)
    batch_size = 1000
    for i in range(0, len(cases), batch_size):
        batch = cases[i:i+batch_size]
        with open(os.path.join(DATA_DIR, f'medical_cases_v2_batch_{i//batch_size+1}.json'), 'w', encoding='utf-8') as f:
    ...
    # 生成古籍条文（50,000条）
    print("\n[2] 生成古籍条文...")
    texts = generate_more_texts(50000)
    for i in range(0, len(texts), batch_size):
        batch = texts[i:i+batch_size]
        with open(os.path.join(DATA_DIR, f'classical_texts_v2_batch_{i//batch_size+1}.json'), 'w', encoding='utf-8') as f:
            json.dump(batch, f, ensure_ascii=False, indent=2)
        total += len(batch)
    ...
    with open(os.path.join(DATA_DIR, 'bulk_v2_stats.json'), 'w', encoding='utf-8') as f:
    ...
    print(f"  现有症状: {len(symptoms)}")
    print(f"  现有药物: {len(drugs)}")
    print(f"  现有方剂: {len(formulas)}")
    print("=" * 60)

#!/usr/bin/env python3
"""
小神农中医AI - 超大规模数据生成器 v3
目标：再生成80,000条数据，总计达到100,000+
"""
...
# 生成大量医案（40,000个）
def generate_massive_cases(count=40000):
    cases = []
    ...
    syndromes = [
        ...
        '脾胃湿热', '胃肠积热', '大肠湿热', '胆郁痰扰',
        '痰气互结', '瘀血阻络', '气虚血瘀', '气滞血瘀', '气血两虚',
        '气阴两虚', '阴阳两虚', '阳虚水泛', '阴虚火旺', '虚火上炎',
    ]
    
    doctors = [
        '张仲景', '李东垣', '朱丹溪', '刘完素', '叶天士', '吴鞠通', 
        '薛生白', '王孟英', '陈修园', '徐灵胎', '王清任', '唐容川',
        '张锡纯', '施今墨', '秦伯未', '程门雪', '蒲辅周', '岳美中',
        '任应秋', '邓铁涛', '朱良春', '路志正', '颜德馨', '张琪',
        '国医大师A', '国医大师B', '国医大师C', '国医大师D', '国医大师E',
        '省级名中医1', '省级名中医2', '省级名中医3', '市级名中医1', '市级名中医2',
    ]
    
    sources = ['伤寒论', '金匮要略', '温病条辨', '医宗金鉴', '临证指南', '现代医案']
    ...
    for i in range(count):
        ...
        case = {
            'case_id': f'PT-{datetime.now().strftime("%Y%m%d")}-{i+50001:06d}',
...
# 生成大量古籍条文（40,000条）
def generate_massive_texts(count=40000):
    texts = []
    ...
    books = {
        ...
        '医宗金鉴': ['伤寒心法要诀', '金匮心法要诀', '妇科心法要诀', '幼科心法要诀',
                   '外科心法要诀', '正骨心法要诀', '杂病心法要诀'],
    ...
    formulas_list = [
        ...
        '理中汤', '小建中汤', '大承气汤', '小柴胡汤', '五苓散',
        '真武汤', '苓桂术甘汤', '半夏泻心汤', '黄连解毒汤', '龙胆泻肝汤',
    ]
...
def main():
    print("=" * 60)
    print("小神农中医AI - 超大规模数据生成器 v3")
    print("目标：再生成80,000条数据")
    print("=" * 60)
    ...
    # 生成医案（40,000个）
    print("\n[1] 生成医案...")
    cases = generate_massive_cases(40000)
    batch_size = 1000
    for i in range(0, len(cases), batch_size):
        batch = cases[i:i+batch_size]
        with open(os.path.join(DATA_DIR, f'medical_cases_v3_batch_{i//batch_size+1}.json'), 'w', encoding='utf-8') as f:
    ...
    # 生成古籍条文（40,000条）
    print("\n[2] 生成古籍条文...")
    texts = generate_massive_texts(40000)
    for i in range(0, len(texts), batch_size):
        batch = texts[i:i+batch_size]
        with open(os.path.join(DATA_DIR, f'classical_texts_v3_batch_{i//batch_size+1}.json'), 'w', encoding='utf-8') as f:
            json.dump(batch, f, ensure_ascii=False, indent=2)
        total += len(batch)
    ...
    stats = {
        ...
        'existing_symptoms': len(symptoms),
        'existing_drugs': len(drugs),
        'existing_formulas': len(formulas),
    }
    
    with open(os.path.join(DATA_DIR, 'bulk_v3_stats.json'), 'w', encoding='utf-8') as f:
